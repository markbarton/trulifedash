    ...
    */
    function setCss(e, css) {
        ...
        for (var p in css) {
            var val = css[p];
            if (isNumber(val)) {
                if (p.match(/width|height|size|left|top|right|bottom'/i)) {
    ...
    var Globalize = (function () {
        ...
        */
        Globalize.formatNumber = function (value, format) {
            ...
            var nf = wijmo.culture.Globalize.numberFormat, f1 = format[0];
            ...
            // get result
            var result = (f1 == 'c' || f1 == 'p') ? Math.abs(value).toFixed(dec) : value.toFixed(dec);
        ...
        Globalize.parseDate = function (value, format) {
            ...
            for (var i = 0; dp && i < dp.length && i < fp.length; i++) {
                switch (fp[i]) {
                    ...
                    case 'tt':
                    case 't':
                        if (dp[i].toLowerCase().indexOf('p') == 0 && hour < 12) {
                            hour += 12;
                        }
                        break;
                    case 'yyyy':
            ...
            // return result
            var d = new Date(year, month, day, hour, min, sec, ms);
        ...
        */
        var CollectionView = (function () {
            ...
            */
            CollectionView.prototype.remove = function (item) {
                ...
                if (index > -1) {
                    ...
                    // issue notifications (item removed or full refresh)
                    if (digest == this._getGroupsDigest(this.groups)) {
            ...
            */
            ...
            // gets a string digest of the current groups (useful for tracking changes)
    ...
    */
    var Tooltip = (function () {
        ...
        */
        ...
        // gets an element from a parameter that may be an element or an element selector
        Tooltip.prototype._getElement = function (e) {
            if (e instanceof HTMLElement) {
                return e;
            }
            if (e instanceof wijmo.Control) {
                return e.hostElement;
            }
            if (wijmo.isString(e)) {
                var jq = $(e);
                if (jq.length > 0) {
    ...
    var Color = (function () {
        ...
        */
        Color.prototype.toString = function () {
            return this.a == 1 ? wijmo.format('rgb({r},{g},{b})', this) : wijmo.format('rgba({r},{g},{b},{a})', this);
        };
        ...
        */
        ...
        Color.prototype._parse = function (color) {
            // let browser convert color into rgba
            ...
            // if the color was valid, we should have an rgb by now
            wijmo.assert(color.indexOf('rgb') == 0 || !color, 'We should have an rgb/rgba value here.');
